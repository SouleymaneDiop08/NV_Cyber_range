...
class TelemetryCollector:
    ...
    async def read_discrete_inputs(self) -> Dict[str, bool]:
        ...
        try:
            # Read all status bits used by railway1.st, including DMD states.
            start, count = 0, 64
    ...
    async def read_holding_registers(self) -> Dict[str, int]:
        ...
        try:
            # Standard setpoints (OpenPLC %MW0/%MW1 = Modbus addr 1024/1025)
            # and Feeder 2 phase currents on %QW0..2.
            for start, count in [(1024, 4), (0, 4)]:
    ...
    def _build_structured_data(
        ...
        return {
            ...
            "trains": {
                "dakar": {
                    "speed": input_registers.get("DKR_MET_VITESSE_TRAIN", 0),
                    "position": input_registers.get("DKR_MET_POSITION_TRAIN", 0),
                },
                "diamniadio": {
                    "speed": input_registers.get("DMD_MET_VITESSE_TRAIN", 0),
                    "position": input_registers.get("DMD_MET_POSITION_TRAIN", 0),
                    "catenary_voltage": input_registers.get("DMD_MET_TENSION_CATENAIRE", 0),
                    "incident_level": input_registers.get("DMD_MET_NIVEAU_INCIDENT", 0),
                    "signal_green": discrete_inputs.get("DMD_ETAT_SIGNAL_VERT", False),
                    "signal_red": discrete_inputs.get("DMD_ETAT_SIGNAL_ROUGE", False),
                    "alarm_critical": discrete_inputs.get("DMD_ALM_CRITIQUE", False),
                    "alarm_low_voltage": discrete_inputs.get("DMD_ALM_TENSION_BASSE", False),
                },
            },
        }
