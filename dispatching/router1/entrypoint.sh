...
echo "Demarrage du routeur R1-R3 (deploiement OT_RANGE_V2)..."
...
ip route add 192.168.20.0/24 via 192.168.30.253 2>/dev/null || true

HEDGEHOG_ENABLE="${HEDGEHOG_ENABLE:-true}"
HEDGEHOG_LOCAL_IP="${HEDGEHOG_LOCAL_IP:-192.168.30.254}"
HEDGEHOG_REMOTE_IP="${HEDGEHOG_REMOTE_IP:-192.168.30.50}"
HEDGEHOG_TUNNEL_KEY="${HEDGEHOG_TUNNEL_KEY:-101}"
HEDGEHOG_TUNNEL_IFACE="${HEDGEHOG_TUNNEL_IFACE:-gretap_l1}"
HEDGEHOG_SENSOR_IP="${HEDGEHOG_REMOTE_IP}"

if [ "${HEDGEHOG_ENABLE}" = "true" ]; then
  echo "Configuration du port mirroring R1 -> ${HEDGEHOG_REMOTE_IP}..."
  ...
  # Ne pas recopier le trafic du capteur lui-meme sur l'interface L3,
  # sinon les paquets GRE encapsules se retrouvent reinjectes et on cree
  # des captures imbriquees inutiles.
